...
def main():
    ...
    parser.add_argument('--num_runs', type=int, default=10, help='Number of times to run the experiment.')
    ...
    for i in range(args.num_runs):
        ...
        model = model_class(in_features=2048, out_features=num_classes, k=args.curvature).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            log_dict = {
                "epoch": current_epoch_num, "train/loss": train_loss, "train/top1_accuracy": train_acc,
            ...
            if current_test_metrics:
                log_dict.update({
                    "test/loss": current_test_metrics["loss"], "test/top1_accuracy": current_test_metrics["top1_accuracy"],
                    "test/top5_accuracy": current_test_metrics["top5_accuracy"], "test/precision": current_test_metrics["precision"],
                    "test/recall": current_test_metrics["recall"], "test/f1_score": current_test_metrics["f1_score"],
                })
            wandb.log(log_dict)
            
            print(f"Epoch {current_epoch_num:02d} | Train Acc: {train_acc:.2f}% | Val Acc: {val_metrics['top1_accuracy']:.2f}%")
            
            if val_metrics["top1_accuracy"] > best_val_acc:
                best_val_acc = val_metrics["top1_accuracy"]
                epochs_no_improve = 0
                print(f"✨ New best val acc: {best_val_acc:.2f}%. Storing metrics from epoch {current_epoch_num}.")
                
                best_epoch_num = current_epoch_num # <-- MODIFIED: Record the best epoch number
                best_epoch_val_metrics = val_metrics
                best_epoch_test_metrics = current_test_metrics
                wandb.summary['best_val_accuracy'] = best_val_acc
                wandb.summary['best_test_accuracy'] = best_epoch_test_metrics["top1_accuracy"]
            else:
                epochs_no_improve += 1
        ...
        all_best_epochs.append(best_epoch_num) # <-- MODIFIED: Append the best epoch number
        
        # Handle cases where training finishes without ever improving
        if best_epoch_val_metrics is None:
            best_epoch_val_metrics = val_metrics
            best_epoch_test_metrics = evaluate(model, test_loader, criterion, device) # Final test eval
    ...
    val_top5_acc_mean, val_top5_acc_std = get_stats(all_best_val_metrics, 'top5_accuracy')
    ...
    test_top5_acc_mean, test_top5_acc_std = get_stats(all_best_test_metrics, 'top5_accuracy')
