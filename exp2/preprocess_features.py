...
def get_datasets(dataset_name: str, data_path: str) -> tuple[Dataset, Dataset, Dataset]:
    ...
    if dataset_name == 'aircraft':
        ...
        train_dataset = datasets.FGVCAircraft(root=data_path, split='train', download=download)
        val_dataset = datasets.FGVCAircraft(root=data_path, split='val', download=download)
        test_dataset = datasets.FGVCAircraft(root=data_path, split='test', download=download)
    elif dataset_name == 'flowers':
        print("Loading Oxford Flowers-102 dataset...")
        train_dataset = datasets.Flowers102(root=data_path, split='train', download=download)
        val_dataset = datasets.Flowers102(root=data_path, split='val', download=download)
        test_dataset = datasets.Flowers102(root=data_path, split='test', download=download)
    elif dataset_name == 'dogs-custom':
        print("Loading custom Stanford Dogs dataset...")
        train_dataset = StanfordDogsCustom(root=data_path, split='train', download=download)
        val_dataset = StanfordDogsCustom(root=data_path, split='val', download=download)
        test_dataset = StanfordDogsCustom(root=data_path, split='test', download=download)
    else:
...
def extract_and_save_features(model, loader, device, feature_path, label_path):
    ...
    with torch.no_grad():
        for images, labels in tqdm(loader, desc=f"Processing {os.path.basename(feature_path)}"):
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess image datasets and save ResNet-50 features.")
    parser.add_argument('--dataset', type=str, required=True, 
                        choices=['aircraft', 'flowers', 'dogs-custom'],
                        help='Name of the dataset to process.')
    parser.add_argument('--data_path', type=str, default='./data', help='Path to download raw datasets.')
    parser.add_argument('--features_path', type=str, default='./features', help='Path to save extracted features.')
    parser.add_argument('--num_epochs', type=int, default=1, help='Epochs for augmented training set generation.')
    ...
    stats_calc_dataset = DatasetTransformer(train_data, transforms.Compose([
    ...
    if args.num_epochs > 1:
        ...
        train_loader = DataLoader(train_dataset_transformed, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        for epoch in range(args.num_epochs):
            print(f"--- Epoch {epoch+1}/{args.num_epochs} ---")
            extract_and_save_features(backbone, train_loader, device,
        ...
        train_loader = DataLoader(train_dataset_transformed, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
        extract_and_save_features(backbone, train_loader, device,
    ...
    for split_name, split_data in [('validation', val_data), ('test', test_data)]:
        ...
        loader = DataLoader(dataset_transformed, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
        extract_and_save_features(backbone, loader, device,
