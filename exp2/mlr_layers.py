...
class Lorentz_fully_connected_forward(nn.Module):
    ...
    def __init__(self, in_features, out_features, k = 0.1, reset_params="kaiming", activation = nn.Identity()):
        super().__init__()
        self.manifold = Lorentz(k)
        self.linear = Lorentz_fully_connected(in_features, out_features, self.manifold, reset_params, activation)
...
class Poincare_MLR_Shimizu_van_Spengler(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, k = 0.1):
        ...
        self.mlr = Poincare_distance2hyperplanes(in_features, out_features, self.manifold)
