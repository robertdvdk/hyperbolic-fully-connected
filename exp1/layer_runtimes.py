...
class Lorentz(nn.Module):
    ...
    def __init__(self, k: float = 0.1, requires_grad=False, constraining_strategy=nn.Identity()):
        super().__init__()
    ...
    def projection_space_orthogonal(self, x):
        ...
        """
        return torch.cat([torch.sqrt(1/self.k() + x.pow(2).sum(-1, keepdim = True)), x], dim=-1)
...
class BdeirLorentzMLR(nn.Module):
    ...
    def forward(self, x, scale_displacement: bool = False):
        ...
        if not scale_displacement:
            ...
            d = 1 / sqrt_mK * torch.abs(torch.asinh(sqrt_mK*alpha/beta))  # Distance to hyperplane
            ...
            beta = torch.sqrt(-w_t**2+torch.norm(w_s, dim=-1)**2)
            alpha = -w_t*x.narrow(-1, 0, 1) + (torch.cosh(sqrtk_a_znorm)*torch.inner(x.narrow(-1, 1, x.shape[-1]-1), self.z))
...
def time_operation_cuda(model, input_tensor, method_name='forward'):
    ...
    if not next(model.parameters()).is_cuda or not input_tensor.is_cuda:
        raise ValueError("Model and input_tensor must be on a CUDA device for accurate timing.")
...
def main():
    ...
    fieldnames = [
        "Model", "In", "Out", "Batch", "Compiled", 
        "Fwd Mean (ms)", "Fwd Std (ms)", "Bwd Mean (ms)", "Bwd Std (ms)"
    ]
    ...
    for index_run, (in_dim, out_dim, batch_size) in enumerate(configs):
        ...
        print(f"  Index: {index_run + 1}/{len_configs} | {100 * (index_run + 1) / len_configs:.2f}% complete")

        try:
            # --- Prepare Synthetic Data ---
            base_tangent_data = torch.randn(batch_size, in_dim, device=device)
            poincare_input = poincare_manifold.expmap0(base_tangent_data.clone()).detach()
            ...
            print(f"  ERROR preparing data for this config: {e}. Skipping to next config.")
        ...
        models_to_test = {
            "Lorentz_FC (MLR)": (Lorentz_fully_connected(in_dim, out_dim, manifold=lorentz_manifold), lorentz_input, 'mlr'),
            "Lorentz_FC cached (MLR)": (Lorentz_fully_connected(in_dim, out_dim, manifold=lorentz_manifold), lorentz_input, 'mlr_cache'),
            "Lorentz_FC + ReLU": (Lorentz_fully_connected(in_dim, out_dim, manifold=lorentz_manifold, activation=F.relu), lorentz_input, 'forward'),
            # "Lorentz_FC + GELU": (Lorentz_fully_connected(in_dim, out_dim, manifold=lorentz_manifold, activation=F.gelu), lorentz_input, 'forward'),
            "Lorentz_FC cached + ReLU": (Lorentz_fully_connected(in_dim, out_dim, manifold=lorentz_manifold, activation=F.relu), lorentz_input, 'forward_cache'),
            ...
            "BdeirLorentzMLR": (BdeirLorentzMLR(in_dim + 1, out_dim, manifold=lorentz_manifold), lorentz_input, 'forward'),
            "ChenLinear (bias=F, norm=F)": (ChenLinear(manifold=lorentz_manifold, in_features=in_dim+1, out_features=out_dim+1, bias=False, normalize=False), lorentz_input, 'forward'),
            "ChenLinear (bias=T, norm=T)": (ChenLinear(manifold=lorentz_manifold, in_features=in_dim+1, out_features=out_dim+1, bias=True, normalize=True, learn_scale=True), lorentz_input, 'forward'),
            "Poincare_Dist2Hyperplanes": (Poincare_distance2hyperplanes(in_dim, out_dim, manifold=poincare_manifold), poincare_input, 'forward'),
            "Poincare_Linear": (Poincare_linear(in_dim, out_dim, manifold=poincare_manifold), poincare_input, 'forward'),
            "PoincareActivation (ReLU)": (PoincareActivation(activation=nn.ReLU(), manifold=poincare_manifold), poincare_input, 'forward'),
            # "PoincareActivation (GELU)": (PoincareActivation(activation=nn.GELU(), manifold=poincare_manifold), poincare_input, 'forward'),
            "Euclidean Linear": (nn.Linear(in_dim, out_dim), euclidean_input, 'forward'),
            "Euclidean Linear no bias": (nn.Linear(in_dim, out_dim, bias = False), euclidean_input, 'forward'),
            "Euclidean Linear + ReLU": (nn.Sequential(nn.Linear(in_dim, out_dim), nn.ReLU()), euclidean_input, 'forward'),
            "Euclidean Linear no bias + ReLU": (nn.Sequential(nn.Linear(in_dim, out_dim, bias = False), nn.ReLU()), euclidean_input, 'forward'),
            ...
            for compiled in [False]:
                ...
                try:
                    ...
                    if compiled:
                        try:
                            if method != 'forward':
                                setattr(model, method, torch.compile(getattr(model, method)))
                            else:
                    ...
                    fw_mean, fw_std, bw_mean, bw_std = time_operation_cuda(model, current_input, method_name=method)
                    
                    result_row.update({
                        "Fwd Mean (ms)": fw_mean, "Fwd Std (ms)": fw_std,
                        "Bwd Mean (ms)": bw_mean, "Bwd Std (ms)": bw_std,
                    })

                except Exception as e:
                    print(f"    ERROR running {name} (Compiled: {compiled}): {e}")
                    result_row.update({
                        "Fwd Mean (ms)": "ERROR", "Fwd Std (ms)": "ERROR",
                        "Bwd Mean (ms)": "ERROR", "Bwd Std (ms)": "ERROR",
                    })
